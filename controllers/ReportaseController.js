...
app.get("/kecamatan/:id_kecamatan", userSession, async (req, res, next) => {
    response.sendResponse(res, await modules.listReportaseKecamatan(Number(req.params.id_kecamatan)));
});

app.get("/:id_reportase", userSession, async (req, res, next) => {
    response.sendResponse(res, await modules.getReportase(Number(req.params.id_reportase)));
});
